...
class XUploadAction extends CAction {
        ...
        if (isset($_GET["_method"]) && $_GET["_method"] == "delete") {
            ...
            if ($_GET["file"][0] !== '.' && Yii::app()->user->hasState($this->stateVariable)) {
                ...
                if ($this->fileExists($userFiles[$_GET["file"]])) {
                    //从数据库删除，2013-11-23 13:51 by yangpeijun
                    $this->deleteFileRecord($userFiles[$_GET["file"]]);
    ...
    protected function handleUploading()
    {
        ...
        if ($model->{$this->fileAttribute} !== null) {
             ...
             * 对上传的文件进行重命名 目前的命名规则是 “时间戳_5位随机数”
             */
            $model->{$this->fileNameAttribute} = time().'_'.mt_rand(10000,99999); 
            $model->{$this->fileNameAttribute} .= ".".$model->file->getExtensionName( );
            ...
            if ($model->validate()) {
                ...
                if ($returnValue === true) {
                    //存储到数据库 2013-11-23 12:05 by yangpeijun
                    $fileRecord = $this->saveUploadToDB($model);
     ...
     * 保存上传的资源信息存储到数据库
     * #author yangpeijun
     * #date 2013-11-23 12:29
     * @param $model
     * @return bool
     */
    private function saveUploadToDB($model) {
        $upload = new SysUpload();
        $form = new FormModel();

        $upload->name = $model->{$this->displayNameAttribute};
        $upload->type = $model->{$this->mimeTypeAttribute};
        $upload->size = $model->{$this->sizeAttribute};
        $upload->url = $model->getThumbnailUrl($this->getPublicPath());
        $upload->delete_url = $this->getController()->createUrl($this->getId(), array(
                                "_method" => "delete",
                                "file" => $model->{$this->fileNameAttribute},
                            ));

        $upload->action = isset($_GET["action"]) ? $_GET["action"] : '';
        $upload->action_id = isset($_GET["action_id"]) ? $_GET["action_id"] : '';

        $upload->account_id = $form->getAccountId();
        $upload->update_time = date('Y-m-d H:i:s');

        if(!$upload->save()) {
            throw new Exception("File info save db failed.");
        }

        return $upload;
    }

    private function deleteFileRecord($file) {
        return SysUpload::model()->find("url='".$file["url"]."'")->delete();
    }
