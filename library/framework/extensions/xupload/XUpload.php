...
class XUpload extends CJuiInputWidget {
    ...
    public function run() {
        ...
        //$this->options['add'] = function(e, data){ $.each(data.files, function (index, file) {alert('Selected file: ' + file.name);});};
        ...
        // Yii::app() -> clientScript -> registerScript(__CLASS__ . '#' . mt_rand(10000,99999), "jQuery('#{$this->htmlOptions['id']}').fileupload({ done:function(e, data){ $.each(data.result, function (index, file) { window.parent.func(file.thumbnail_url); } );} });", CClientScript::POS_READY);
