$(function () {
    $.fn.datePicker = function (options) {
        ...
        datePicker(nowYear, nowMonth, nowWeek, nowLastday); //初始化为当前日期
        $("#month").html(nowMonth);
        $("#year").html(nowYear);
        //上一月绑定点击事件
        $("#preMonth").click(function () {
            ...
            datePicker(year, month, week, lastday);
        });
        //下一个月的点击事件
        ...
        //初始化日历
        function datePicker(year, month, week, lastday) {
                $("#year").val(year);
                $("#month").val(month)
                ...
                for (var j = week; j < 7; j++) {
                    if (n == today && isToday) {
                        ...
                        /*table.rows[i].cells[j].innerHTML = n+"<span class='order'></span>";*/
                    } else {
        ...
        $mhInput.live(options.Event, function (e) {
            ...
            var iof = $(this).offset();
            $(".calender").css({
                "left": iof.left + options.Left,
                "top": iof.top + options.Top
            });
            $(".calender").show();
        });
        //当鼠标离开控件上面的时候延迟3秒关闭
