;(function ($) {
    $.cookie = function (key, value, options) {
        ...
        if (arguments.length > 1 && val !== "[object Object]") {
            ...
            expiresTime != '' && (num = expiresTime.substring(1, expiresTime.length) * 1, type = expiresTime.substring(0, 1), strsec = {
                's': num * 1000,
                'h': num * 60 * 60 * 1000,
                'd': num * 24 * 60 * 60 * 1000
            }[type], exp.setTime(exp.getTime() + strsec * 1), expiresTime = ";expires=" + exp.toGMTString());
