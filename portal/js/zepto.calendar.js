    ...
    $.fn.almanac = function (options) {
        ...
        var Almanac = {
                ...
                for (i = 0; i < 42; i++) {
                    ...
                    if (sD > -1 && sD < cld.length) {  //日期内
                        ...
                        if (cld[sD].isToday) {      // 今日
                            eDay.addClass("today").addClass("today_size").addClass("choose_day");         //今日颜色

                            this.showAlmanacDetail(eDay);   // 显示右侧黄历

                            if(cld[sD].week == "六"){//解决周六显示问题
                                eDay.removeClass("today_size");
                            }
                        }
                    ...
                    for (var j = 0; j < 7; j++) {
                        ...
                        if (!hasli) {
                            ...
                            li = $('<li/>').attr({
                                ...
                                .append('<div class="date_item"><span class="solar" id="SD' + gNum + '"></span><span class="lunar" id="LD' + gNum + '"</span></div>')
            ...
            initRiliIndex: function () {
                if(global.currYear == -1)
                    global.currYear = this.today.tY();    // 当前年
                if(global.currMonth == -1)
            ...
            initControlBar: function () {
                // 设置选中的年为当前的年
                var tM = global.currMonth + 1;
                $(".year").text(global.currYear + '年');
                // 设置年的选项
                var htmlYear = "";
                for (i = 1901; i <= 2100; i++) {
                    if (i == global.currYear)
            ...
            nextMonth: function () {
                ...
                if (month == 12) {
                    ...
                    if (year <= this.maxYear) {
                        month = 0;
                        this.setYear(year);
                        localStorage.setItem('year',this.currYear + 1);
                        localStorage.setItem('month',this.currMonth + 1);
                    } else {
                        month = 11;
                        localStorage.setItem('year',this.currYear + 1);
                        localStorage.setItem('month',this.currMonth + 1);
        ...
        return this.each(function () {
            ...
            /*年份递减*/
            $("#prev_y").unbind('click').bind('click', function () {
                DateSelection.goPrevYear();
            });
            /*年份递加*/
            $("#next_y").unbind('click').bind('click', function () {
                DateSelection.goNextYear();
            });

            /*月份递减*/
            $("#prev_m").unbind('click').bind('click', function () {
                DateSelection.goPrevMonth();
                //global.changeMonth(DateSelection.currYear, DateSelection.currMonth);
            });

            /*月份递加*/
            $("#next_m").unbind('click').bind('click', function () {
