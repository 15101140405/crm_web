...
{
    $html = "<li class=\"ulist_item\" order-id=\"" . $result['id'] . "\" order-type=\"" . $result['order_type'] . "\">" .
        "<span class=\"order_status \" order_status=\"". $result['order_status'] ."\"></span>" .
        /*"<span class=\"order_status " . $result['order_color'] . "\">" . $result['order_stat'] . "</span>" .*/
        "<div class=\"order_info\">" .
        "<p class=\"customer\">" . $result['order_name'] . "</i></p>" .
       /* "<p class=\"customer\"><i class=" . $result['order_new'] . ">" . $result['account_id'] . "</i></p>" .*/
        "<p class=\"desc\">" . $result['order_date'] . "</p>" .
...
<article id="homepage">
    <div class="tool_bar fixed">
        ...
        $arr_hotel = array(//+++++++++++++++++++++++++++所需数据
            '0' => array(
                'hotel_name' => '1xxx酒店(酒店名称)',
                'staff_hotel_id' => '1',
                'select' => '' //选择的酒店
            ),
            '1' => array(
                'hotel_name' => '2伊缘圆婚礼主题酒店',
                'staff_hotel_id' => '2',
                'select' => ''
            )
        );
...
<script>
    ...
    $(function () {
        ...
        $("#wed").on("click", function () {
            //打印订单状态筛选条
            var html_tab = '<div class="tab_module fixed3" id="second_tab"><p class="tab_btn act" id="second_total"><span>全部</span></p><p class="tab_btn" id="non_front"><span>未交订金</span></p><p class="tab_btn" id="front"><span>已交订金</span></p><p class="tab_btn" id="middle"><span>付中期款</span></p><p class="tab_btn" id="end"><span>已完成</span></p></div>'
            $("#second_tab").remove();
        ...
        function plan_jump (order_type,order_status,order_id) { 
            location.href = "<?php echo $this->createUrl('finance/wedcostcalculate');?>&order_id=" + order_id + "&from=order";
        }
